...
public class AppEngineDeployInfoTest {
  ...
  private static final String XML_WITHOUT_PROJECT_ID = XML_PREFIX + XML_SUFFIX;
  private static final String XML_WITHOUT_VERSION = XML_PREFIX + PROJECT_ID + XML_SUFFIX;
  private static final String XML_WITH_VERSION_AND_PROJECT_ID = XML_PREFIX + PROJECT_ID + VERSION + XML_SUFFIX;
  ...
  private File createFileWithContent(String xmlWithoutProjectId) throws IOException {
    ...
    try (PrintStream stream = new PrintStream(tempFile)) {
      stream.print(xmlWithoutProjectId);
    }
    return tempFile;
  }
